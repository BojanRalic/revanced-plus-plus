    ...
    apply {
        forEachInstructionAsSequence(
            match = match@{ _, _, instruction, instructionIndex ->
                val reference = instruction.getReference<FieldReference>() ?: return@match null
                if (reference.definingClass != BUILD_CLASS_DESCRIPTOR) return@match null

                replacements[reference.name]?.let { instructionIndex to it }
            },
