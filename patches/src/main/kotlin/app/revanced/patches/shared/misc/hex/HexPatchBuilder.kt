            ...
            HexPatchBuilder().apply(block).groupBy { it.targetFilePath }
                .forEach { (targetFilePath, replacements) ->
                    ...
                    FileChannel.open(
                        ...
                        replacements.forEach { it.replacePattern(buffer) }
                        buffer.force()
                    }
                }
        }
...
class Replacement(
    ...
    /**
     * Replaces the [bytes] with the [replacementBytes] directly in the memory-mapped [buffer].
     *
     * @param buffer The memory-mapped buffer of the file to make the changes in.
     */
    internal fun replacePattern(buffer: MappedByteBuffer) {
        val startIndex = indexOfPatternIn(buffer)
        ...
        replacementBytesPadded.forEachIndexed { i, byte ->
            buffer.put(startIndex + i, byte)
        }
    }

    /**
     * Returns the index of the first occurrence of [bytes] in the [buffer]
     * using the Boyer-Moore algorithm.
     *
     * @param buffer The memory-mapped buffer to search in.
     *
     * @return The index of the first occurrence of the [bytes] in the buffer or -1
     * if the [bytes] is not found.
     */
    private fun indexOfPatternIn(buffer: MappedByteBuffer): Int {
        val needle = bytes
        val bufferSize = buffer.limit()
        val right = IntArray(256) { -1 }

        for (i in needle.indices) right[needle[i].toInt().and(0xFF)] = i

        var skip: Int
        for (i in 0..bufferSize - needle.size) {
            skip = 0

            for (j in needle.size - 1 downTo 0) {
                if (needle[j] != buffer.get(i + j)) {
                    skip = max(1, j - right[buffer.get(i + j).toInt().and(0xFF)])
                    break
                }
            }
