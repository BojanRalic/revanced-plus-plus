    ...
    apply {
        forEachInstructionAsSequence(
            match = match@{ _, _, instruction, instructionIndex ->
                if (instruction !is ReferenceInstruction) return@match null
                val reference = instruction.reference as? StringReference ?: return@match null

                if (reference.string != "android.support.customtabs.action.CustomTabsService") return@match null

                instructionIndex to (instruction as OneRegisterInstruction).registerA
            },
