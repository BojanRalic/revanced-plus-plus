...
internal val playerControlsResourcePatch = resourcePatch {
    ...
    apply {
        ...
        addTopControl = { resourceDirectoryName ->
            ...
            val targetDocument = document("res/layout/youtube_controls_layout.xml")

            "RelativeLayout".copyXmlNode(sourceDocument, targetDocument).use {
                // Find the leftmost button in the added control row — the element whose android:id
                // is not referenced by any sibling via android:layout_toStartOf.
                val sourceChildren = sourceDocument.getElementsByTagName("RelativeLayout").item(0).childNodes
                val referencedToStartOf = buildSet<String> {
                    for (i in 0 until sourceChildren.length) {
                        val child = sourceChildren.item(i)
                        if (!child.hasAttributes()) continue
                        child.attributes.getNamedItem("android:layout_toStartOf")?.nodeValue?.let(::add)
                    }
                }
                val leftmostButtonId = run {
                    for (i in 0 until sourceChildren.length) {
                        val child = sourceChildren.item(i)
                        if (!child.hasAttributes()) continue
                        val id = child.attributes.getNamedItem("android:id")?.nodeValue ?: continue
                        // IDs may use @+id/ or @id/; check both forms.
                        if (id !in referencedToStartOf && id.replace("@+id/", "@id/") !in referencedToStartOf) {
                            return@run id
                        }
                    }
                    throw PatchException("Could not determine leftmost button ID in $resourceFileName")
                }

                targetDocument.childNodes.findElementByAttributeValueOrThrow(
                    "android:id",
                    "@id/player_video_heading",
                ).attributes.getNamedItem("android:layout_toStartOf").nodeValue = leftmostButtonId
            }
        }
