...
private fun Method.findFreeRegisters(
    ...
    val (offsetArray, payloadsByOffset) = buildInstructionOffsetData()
    ...
    val freeRegisters = findFreeRegistersInternal(
        ...
        payloadsByOffset = payloadsByOffset,
    )
 ...
 * @param payloadsByOffset Map from absolute code offset to switch payload instruction.
...
private fun Method.findFreeRegistersInternal(
    ...
    payloadsByOffset: Map<Int, SwitchPayload>,
): List<Int> {
    ...
    for (i in startIndex until instructions.count()) {
        ...
        if (instruction.isSwitchInstruction) {
            ...
            val switchOffset = offsetArray[i]
            val payloadOffset = switchOffset + (instruction as OffsetInstruction).codeOffset
            val payload = payloadsByOffset[payloadOffset]

            if (payload == null || payload.switchElements.isEmpty()) {
                // No payload found or no cases — treat as leaf node.
                return freeRegisters.toList()
            }

            val usedRegistersList = usedRegisters.toList()

            // Explore every switch case branch.
            val caseBranchResults = payload.switchElements.map { element ->
                val targetOffset = switchOffset + element.offset
                val targetIndex = findInstructionIndexByOffset(targetOffset, offsetArray)
                findFreeRegistersInternal(
            ...
            // Also explore the fall-through path (when no case key matches).
            val fallThroughResults = findFreeRegistersInternal(
                startIndex = i + 1,
                numberOfFreeRegistersNeeded = numberOfFreeRegistersNeeded,
                currentDepth = currentDepth + 1,
                foundFreeRegistersAtIndex = foundFreeRegistersAtIndex,
            ...
            // Only registers that are free in ALL paths (cases + fall-through) can be used.
            val allBranchResults = caseBranchResults + listOf(fallThroughResults)
            val commonFreeRegisters = allBranchResults.reduce { acc, list ->
                acc.intersect(list.toSet()).toList()
            }

            return (freeRegisters + commonFreeRegisters).toList()
        }
...
/**
 * Builds the instruction offset data for this method.
 *
 * @return A pair of:
 *   - An [IntArray] mapping instruction index → absolute code offset (-1 for payload instructions).
 *   - A [Map] from absolute code offset → [SwitchPayload] for switch payload instructions.
 */
private fun Method.buildInstructionOffsetData(): Pair<IntArray, Map<Int, SwitchPayload>> {
    val instructionList = instructions.toList()
    val count = instructionList.size
    val offsetArray = IntArray(count) { -1 }
    val payloadsByOffset = mutableMapOf<Int, SwitchPayload>()
    var currentOffset = 0
    ...
    for (i in instructionList.indices) {
        ...
        } else if (instruction is SwitchPayload) {
            payloadsByOffset[currentOffset] = instruction
        }

        // Use codeUnits for all instructions (including variable-size payloads).
        val sizeInCodeUnits = if (format.size > 0) {
            format.size / 2
        } else {
            instruction.codeUnits
        }
        currentOffset += sizeInCodeUnits
    }

    return Pair(offsetArray, payloadsByOffset)
}
